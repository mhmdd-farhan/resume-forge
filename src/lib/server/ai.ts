...
import type { GitHubProfile, GitHubRepoDetail, LinkedInProfile } from '$lib/server/parser';
import type { Resume, ResumeScore } from '$lib/types';

const N8N_WEBHOOK_URL =
	env.N8N_WEBHOOK_URL ?? 'https://n8n.gloapp.my.id/webhook/resume-generator';
const N8N_BASIC_AUTH = Buffer.from(env.N8N_BASIC_AUTH ?? '').toString('base64');
...
function formatCandidateContext(params: {
	...
	if (githubProfile) {
		...
		if (githubProfile.topProjects.length > 0) {
			...
			githubProfile.topProjects.forEach((p) => {
				profileLines.push(
					`- ${p.name}${p.language ? ` (${p.language})` : ''}${p.stars > 0 ? ` ⭐${p.stars}` : ''}: ${p.description ?? 'No description'}`
				);
			});
		}
	...
	if (githubRepos.length > 0) {
		const repoLines: string[] = [
			'## GITHUB REPOSITORIES (PRIMARY SOURCE for project descriptions — extract real tech stacks, features, and highlights from each README)'
		];
