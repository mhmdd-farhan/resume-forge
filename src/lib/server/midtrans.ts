...
import { PLANS, type PaidPlanType } from '$lib/server/plans';
...
export const MIDTRANS_ENV =
	env.MIDTRANS_ENV === 'production' ? 'production' : 'sandbox';
export const MIDTRANS_SERVER_KEY = env.MIDTRANS_SERVER_KEY ?? '';
export const MIDTRANS_CLIENT_KEY = env.MIDTRANS_CLIENT_KEY ?? '';
...
export async function createSnapTransaction(params: {
	...
	if (!res.ok || !data.redirect_url) {
		throw new Error(
			(data as Record<string, unknown>).status_message ||
				`Midtrans Snap request failed (HTTP ${res.status}).`
		);
	}

	return {
		token: (data as Record<string, string>).token as string,
		redirectUrl: (data as Record<string, string>).redirect_url as string
	};
}
